...
#
# Purpose: Visualize adjusted results
#
...
palette <- c("Truth"="black", "IPCW"="#e7298a", 
             "IPOW"="#377eb8", "MI"="#d95f02")
palette.bw <- c("Truth"="white", "IPCW"="#bbbbbb", 
                "IPOW"="#777777", "MI"="#333333")
...
for (model in c("dag1", "dag2.1", "dag2.2", "dag2.3", "dag2.4",
  ...
  if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
    list <- c("Truth", "IPCW (Z)", "IPOW (Z)", "MI (Z)")
    
  } else {
    list <- c("Truth", "IPCW", "IPOW", "MI")
...
if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
  res.risk.t$approach <- recode(res.risk.t$approach, `IPCW (Z)`="IPCW",  `IPOW (Z)`="IPOW", `MI (Z)`="MI")
...
if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
  res.rate.t$approach <- recode(res.rate.t$approach, `IPCW (Z)`="IPCW",  `IPOW (Z)`="IPOW", `MI (Z)`="MI")
...
if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
  res.risk.r$approach <- recode(res.risk.r$approach, `IPCW (Z)`="IPCW",  `IPOW (Z)`="IPOW", `MI (Z)`="MI")
...
if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
  res.rate.r$approach <- recode(res.rate.r$approach, `IPCW (Z)`="IPCW",  `IPOW (Z)`="IPOW", `MI (Z)`="MI")
...
if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
  res.risk.p$approach <- recode(res.risk.p$approach, `IPCW (Z)`="IPCW",  `IPOW (Z)`="IPOW", `MI (Z)`="MI")
...
if (model %in% c("dag3", "dag3.1", "dag3.2", "dag3.3", "dag4", "dag4.1")) {
  res.rate.p$approach <- recode(res.rate.p$approach, `IPCW (Z)`="IPCW",  `IPOW (Z)`="IPOW", `MI (Z)`="MI")
...
jpeg(paste0("../figures/", model, "/", model, "_adj.jpeg"), height=17, width=17, units="cm", res=300)
  print(panel)
dev.off()
...
jpeg(paste0("../figures/", model, "/", model, "_adj_bw.jpeg"), height=17, width=17, units="cm", res=300)
  print(panel2)
dev.off()
